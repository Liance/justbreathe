	...
	public static class Core_Combine {
		...
		public class RendererList {
			...
			public class RendererData {
				...
				public RendererData (Renderer rd) {
					...
					texture = mat ? mat.mainTexture as Texture2D : null;
		...
		public static Result Combine (Transform root, bool combineMesh, System.Action<string, float> progressLog) {
			...
			var result = GetResult(matMap, texture, newUV, combineMesh);
		...
		private static void AddToMap (Dictionary<MaterialData, RendererList> matMap, Renderer[] renders) {
			for (int i = 0; i < renders.Length; i++) {
				...
				foreach (var pair in matMap) {
					if (pair.Key.SameMaterial(matData)) {
						var rData = new RendererList.RendererData(renders[i]);
						if (rData.Object) {
					...
					matMap.Add(
						...
						new RendererList() {
							List = new List<RendererList.RendererData>() {
								new RendererList.RendererData(renders[i])
							}
						}
					);
				}
