namespace VoxeltoUnity {
	...
	public class UnlimitiedMesh {
		...
		// API
		public UnlimitiedMesh (List<Vector3>[] verticess, List<Vector2>[] uvss, List<BoneWeight> boneWeights = null) {

			if (verticess == null || uvss == null || verticess.Length != uvss.Length) { return; }

			Meshs = new List<Mesh>();
			MaterialIndex = new List<int>();

			for (int matIndex = 0; matIndex < verticess.Length; matIndex++) {
		...
		public UnlimitiedMesh (List<Vector3> vertices, List<Vector2> uvs, List<BoneWeight> boneWeights = null) : this(
			new List<Vector3>[] { vertices }, new List<Vector2>[] { uvs }, boneWeights
		) { }
